...
class ReportingMerge():
    ...
    def _create_database_connections(self, db_paths: tuple = None):
        if db_paths is not None:
            if len(db_paths) > 1:
                ...
                # Sort the connections from oldest to newest
    ...
    def _find_db_with_table(self, table_name):
        """Find the first database that contains the specified table."""
    ...
    def _move_all_table_data(self):
        ...
        for table in self.all_tables:
            if table not in self.merge_tables:
                # Find a database that has this table
                source_conn = self._find_db_with_table(table)
                
                if source_conn is None:
                    print(f"Warning: Table {table} not found in any source database!")
                    continue
                
                # Create table in result db first
                if not self.table_exists(table):
                    create_table_sql = self._generate_create_table_sql(source_conn, table)
                    cursor = self.result_db.cursor()
    ...
    def _move_merge_table_data(self):
        ...
        for i, db_conn in enumerate(self.db_connections):
            for table in self.merge_tables:
                # get table data using direct pyodbc
                cursor = db_conn.cursor()
                cursor.execute(f"SELECT * FROM {table}")
                columns = [column[0] for column in cursor.description]
                data = cursor.fetchall()
    ...
    def close(self):
        ...
        try:
            ...
            # Use sys.exit instead of taskkill
            import sys
            sys.exit()
